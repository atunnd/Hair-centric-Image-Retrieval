...
def wait_for_best_gpu(min_vram=4000, check_interval=30):
    ...
    while True:
        ...
        if best_free >= min_vram and best_id != 1:
...
if __name__ == "__main__":
    device = wait_for_best_gpu(min_vram=39000, check_interval=2)  # wait until 20GB free
    os.system(f"CUDA_VISIBLE_DEVICES={device.split(':')[-1]} sh scripts/ablation_study/pretrain_simclr_neg_supervised_randomly_hardnegative.sh")